...
from radioco.global_settings.models import SiteConfiguration
from radioco.programmes.models import CONTRIBUTOR, Episode, Podcast, Programme, Role
...
def create_example_data():
    ...
    Schedule.objects.get_or_create(slot=slot, type="L", recurrences=recurrences)

    Schedule.objects.get_or_create(
        slot=slot, type="R", recurrences=recurrences_repetition
    )
    ...
    for username_counter in range(1, 6):
        ...
        user, created = User.objects.get_or_create(
            username=f"user_{username_counter}",
            defaults={"first_name": titles[username_counter]},
        )
        user.userprofile.bio = synopsis
        user.userprofile.avatar = f"defaults/example/user_{username_counter}.jpg"
    ...
    for programme_counter in range(1, 5):
        programme, created = Programme.objects.get_or_create(
            ...
            synopsis=synopsis,
            language="en",
            photo=f"defaults/example/radio_{programme_counter + 1}.jpg",
            current_season=7,
            category="News & Politics",
        )
        ...
        for season in range(1, 8):
            for number in range(1, 6):
                Episode.objects.get_or_create(
                    title=f"Episode {number}",
                    programme=programme,
                    summary=f"Summary Season {season}, Number {number}: {synopsis}",
                    season=season,
                    number_in_season=number,
                )
