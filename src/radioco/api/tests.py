...
class TestSerializers(TestDataMixin, TestCase):
    ...
    def test_episode_programme(self):
        ...
        self.assertEqual(serializer.data["programme"], "/api/2/programmes/classic-hits")
    ...
    def test_transmission(self):
        ...
        self.assertEqual(data["programme"]["name"], "Classic hits")
        self.assertEqual(data["episode"]["title"], "Episode 1")
        self.assertEqual(data["type"], "L")
        self.assertEqual(data["schedule"], 6)


class TestAPI(TestDataMixin, APITestCase):
    def setUp(self):
        admin = User.objects.create_user(username="klaus", password="topsecret")
        admin.user_permissions.add(Permission.objects.get(codename="add_schedule"))
        admin.user_permissions.add(Permission.objects.get(codename="change_schedule"))
    ...
    def test_transmission_invalid_input(self):
        with self.assertRaises(ValidationError):
            self.client.get("/api/2/transmissions", {"after": "invalid date format"})
    ...
    def test_transmission_now(self):
        ...
        self.assertListEqual(
            ...
            [("Classic hits", "2015-01-06T14:00:00+01:00")],
        )
