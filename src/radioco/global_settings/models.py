...
class SingletonModelManager(models.Manager):
    def get(self, *args, **kwargs):
        obj, _ = super().get_or_create(**kwargs)
        return obj
...
class SiteConfiguration(SingletonModel):
    ...
    twitter_address = models.CharField(
        max_length=255, verbose_name=_("Twitter address"), blank=True, null=True
    )
    facebook_address = models.CharField(
        max_length=255, verbose_name=_("facebook address"), blank=True, null=True
    )
...
class PodcastConfiguration(SingletonModel):
    ...
    def recorder_token(self):
        if hasattr(settings, "USERNAME_RADIOCO_RECORDER"):
            ...
            return _(
                "Variable USERNAME_RADIOCO_RECORDER doesn't exist in your settings file"
            )
