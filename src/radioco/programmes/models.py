...
class Programme(models.Model):
    ...
    language = models.CharField(
        default=PROGRAMME_LANGUAGES[0][0],
...
class EpisodeManager(models.Manager):
    ...
    def unfinished(self, programme, after=None):
        ...
        episodes = Episode.objects.filter(
            Q(programme=programme) & (Q(issue_date__gte=after) | Q(issue_date=None))
        )
        yield from episodes.order_by("season", "number_in_season")
...
class Episode(models.Model):
    ...
    def __str__(self):
        return f"{self.season:d}x{self.number_in_season:d} {self.title or self.programme.name:s}"


class Participant(models.Model):
    person = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name=_("person"))
    episode = models.ForeignKey(
        Episode, on_delete=models.CASCADE, verbose_name=_("episode")
    )
    role = models.CharField(
        default=NOT_SPECIFIED, verbose_name=_("role"), choices=ROLES, max_length=2
    )
...
class Podcast(models.Model):
    episode = models.OneToOneField(
        Episode, on_delete=models.CASCADE, primary_key=True, related_name="podcast"
    )
